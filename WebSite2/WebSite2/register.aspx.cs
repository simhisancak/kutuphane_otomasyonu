...
public partial class register : System.Web.UI.Page
{
    ...
    protected void check(string usrname, string usrpsw , string usrmail)
    {
        ...
        {
            sqlcon.Open();
            string query = "select * from users where user_name=@username or str1=@mail";

            SqlCommand sqlcmd2 = new SqlCommand(query, sqlcon);
            sqlcmd2.Parameters.AddWithValue("@username", usrname.Trim());
            sqlcmd2.Parameters.AddWithValue("@mail", usrmail.Trim());
            SqlDataReader dr = sqlcmd2.ExecuteReader();
            if (dr.Read())
            {
                Label1.Text = "Kullanıcı Adı veya Mail Mevcut";
                Label2.Text = "";
                txtpsw.Text = "";
                txtmail.Text = "";
            }
    ...
    protected void user_register(string usrname, string usrpsw , string usrmail)
    {
        ...
        using (SqlConnection sqlcon = new SqlConnection(connectionstring))
        {
            ...
            sqlcmd.Parameters.AddWithValue("@speed1", 50000);
            sqlcmd.Parameters.AddWithValue("@speed2", 50000);
            sqlcmd.Parameters.AddWithValue("@speed3", 50000);
            sqlcmd.Parameters.AddWithValue("@speed4", 50000);
            sqlcmd.Parameters.AddWithValue("@speed5", 50000);
            sqlcmd.Parameters.AddWithValue("@speed6", 50000);
            sqlcmd.Parameters.AddWithValue("@speed7", 50000);
            sqlcmd.Parameters.AddWithValue("@speed8", 50000);
            sqlcmd.Parameters.AddWithValue("@speed9", 50000);
